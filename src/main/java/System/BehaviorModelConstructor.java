...
//        apis.add("turnOff");
    ...
    private static String executeApi(String api, CoffeeMachine_V1 device) {
        switch (api) {
            case "turnOn":
                device.turnOn();
...
//                LOGGER.info(device.toString());
    ...
    private static void generateGraph(Set<Edge> edges) {
        CoffeeMachine_V1 device = new CoffeeMachine_V1();
...
//            LOGGER.info("Current state: " + state);
            CoffeeMachine_V1 currentCM = CoffeeMachine_V1.fromString(state);
//            CoffeeMachine_V1 currentCM = CoffeeMachine_V1.fromString(state);
            String nextState = executeApi(api, currentCM);
            if (!nextState.equals("skip")){
                CoffeeMachine_V1 nextCM = CoffeeMachine_V1.fromString(nextState);
...
//                    LOGGER.info("State is the same, breaking.");
                        ...
                        nextCM = CoffeeMachine_V1.fromString(_nextState);
...
//        Set<Integer> sizeHistory = new LinkedHashSet<>();
        ...
        for (int i = 0; i < 100; i++) {
            ...
            BehaviorModelConstructor test = new BehaviorModelConstructor();
