...
public class Yeelight_p_Model {
    ...
    private static String executeApi(String api, ProcessBuilder pb) throws IOException, InterruptedException {
        switch (api) {
            case "turnOn":
                return Yeelight_p.executePythonScript(pb, "src/main/python/yeelight/on.py");
            case "turnOff":
                return Yeelight_p.executePythonScript(pb, "src/main/python/yeelight/off.py");
            case "setBrightness":
                return Yeelight_p.executePythonScript(pb, "src/main/python/yeelight/setBrightness.py", "50");
            case "setRGB":
                return Yeelight_p.executePythonScript(pb, "src/main/python/yeelight/setRGB.py", "50", "50", "50");
            case "reset":
                return Yeelight_p.executePythonScript(pb, "src/main/python/yeelight/reset.py");
            default:
                return "Error";
        }
    }

    private static String resetToCurrentState(YeelightStatus currentStatus, String api, ProcessBuilder pb) throws IOException, InterruptedException {
        executeApi("reset", pb);
        LOGGER.info(currentStatus);
        executeApi("turnOn", pb);
        int[] rbg = currentStatus.getRgb();
        int brightness = currentStatus.getBrightness();
        if (brightness != 1) {
            executeApi("setBrightness", pb);
        }
        if (rbg[0] != 255 && rbg[1] != 255 && rbg[2] != 255) {
            executeApi("setRGB", pb);
        }
    ...
    private static void generateGraph(Set<Edge> edges, ProcessBuilder pb) throws IOException, InterruptedException {
        YeelightStatus device = YeelightStatus.parse(executeApi("reset", pb));
    ...
    private static void exploreState(String state, String systemState, Set<Edge> edges, ProcessBuilder pb) throws IOException, InterruptedException {
        ...
        for (String api : apis) {
            YeelightStatus currentState = YeelightStatus.fromString(state);
            YeelightStatus nextState = YeelightStatus.parse(resetToCurrentState(currentState, api, pb));
            if (!nextState.equals(illegalState)){
...
//        dotContent.append("\t__start0 -> s0;\n\n");
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            Yeelight_p_Model test = new Yeelight_p_Model();
...
//        }
        ...
        String dotFile = "src/main/java/System_q/realDevice/yeelight_output.dot";
...
//        });
        ...
        String fileName = "src/main/java/System_q/model_p/Yeelight.json";
